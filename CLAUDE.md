...
- make sure to include a «to do» list in the plan files, so that it’s easy to understand what steps are going to be taken in the implementation. Not a checklist, a «to do list»
This means that the end of the todo list in any plan file contains:
```
...
When creating a commit name after implementing a plan - do NOT mention which phase of the plan you’re in. The commit name should only contain the name. References to which phase of the plan were implemented in this commit should be in the commit message, not in the header line of the commit.
