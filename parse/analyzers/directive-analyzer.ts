...
export function analyzeDirective(directive: Directive, analyzer: SemanticAnalyzer): DirectiveSemanticData | null {
  ...
  switch (keyLower) {
    ...
    // ============================================================================
    case "abcls-voices":
      return parseAbclsVoicesDirective(directive, analyzer);
...
/**
 * Parses %%abcls-voices directive (voice filter for ABC Language Server)
 *
 * Syntax: %%abcls-voices show V1 V2 V3
 *         %%abcls-voices hide V1 V2
 *
 ...
 */
function parseAbclsVoicesDirective(directive: Directive, analyzer: SemanticAnalyzer): DirectiveSemanticData | null {
  ...
  if (directive.values.length < 2) {
    analyzer.report("abcls-voices directive expects mode (show/hide) and at least one voice ID", directive);
    return null;
  }
  ...
  if (!isToken(modeToken) || modeToken.type !== TT.IDENTIFIER) {
    analyzer.report("abcls-voices directive expects 'show' or 'hide' as first parameter", directive);
    return null;
  }

  const modeLower = modeToken.lexeme.toLowerCase();
  if (modeLower !== "show" && modeLower !== "hide") {
    analyzer.report(`abcls-voices directive mode must be 'show' or 'hide', got '${modeToken.lexeme}'`, directive);
    return null;
  }
  ...
  for (let i = 1; i < directive.values.length; i++) {
    const value = directive.values[i];
    if (!isToken(value)) {
      analyzer.report("abcls-voices directive expects voice IDs as parameters", directive);
      return null;
    }
    voiceIds.push(value.lexeme);
  }

  const data: AbclsVoicesDirectiveData = {
