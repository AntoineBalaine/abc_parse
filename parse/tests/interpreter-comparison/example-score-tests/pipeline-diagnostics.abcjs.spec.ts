/**
 * pipeline-diagnostics.abcjs.spec.ts
 *
 * Diagnostic test to identify which files cause hangs in the abcjs parser.
 * Because we need to verify that the hangs are not coming from abcjs itself,
 * we test only the abcjs parser on all example files.
 */
...
describe("Pipeline Diagnostics - abcjs Parser", function () {
  ...
  it("should successfully parse excluded files with abcjs without hanging", function () {
    ...
    for (const filePath of abcFiles) {
      ...
      try {
        ...
        // Because abcjs parser crashes are possible, we catch them and record
        errorCount++;
    ...
    console.log(`\n=== abcjs Parser Test Summary (Excluded Files Only) ===`);
    ...
    if (errorCount > 0) {
      console.log(`abcjs parser errors: ${errorCount}`);
      console.log(`\nFailed files:`);
    ...
    if (failures.length > 0) {
      console.log(`\n${failures.length} excluded files failed abcjs parsing`);
    } else {
      console.log(`\nAll excluded files parsed successfully with abcjs!`);
    }
  });
});
