/**
 * pipeline-diagnostics.scanner.spec.ts
 *
 * Diagnostic test to identify which files cause hangs in the Scanner step.
 * Because we need to isolate the source of infinite loops,
 * we test only the Scanner on all example files.
 */
...
describe("Pipeline Diagnostics - Scanner", function () {
  ...
  it("should successfully scan all example files without hanging", function () {
    ...
    for (const filePath of abcFiles) {
      ...
      try {
        ...
        // Because scanner crashes are possible, we catch them and record
        errorCount++;
    ...
    if (failures.length > 0) {
      throw new Error(`${failures.length} files failed scanner processing`);
    }
  });

  it("should successfully scan excluded files without hanging", function () {
    let processedCount = 0;
    let errorCount = 0;
    const failures: string[] = [];
    ...
    console.log(`\n=== Scanner Test Summary (Excluded Files Only) ===`);
    ...
    if (failures.length > 0) {
      console.log(`\n${failures.length} excluded files failed scanner processing`);
    } else {
      console.log(`\nAll excluded files scanned successfully!`);
    }
  });
});
