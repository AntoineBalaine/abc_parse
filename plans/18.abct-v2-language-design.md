...
The Zig-style capture `|c|` binds the matched node to a variable. The guard uses the same pipeline language. The right side of `=>` produces the replacement. Unmatched nodes pass through unchanged. The strategy (`topdown`, `bottomup`, `oncetd`, `alltd`) handles tree traversal.

Because the strategy handles recursion, the user never writes manual `next`-pointer traversal. They only write the interesting cases.
...
- Pattern matching with strategies separates "what to do" (rules) from "where to do it" (traversal).
...
- Strategy application: `topdown rule`, `bottomup rule`, `oncetd rule`, `alltd rule` — controls where a pattern-matching rule is applied in the tree.
...
- `topdown <rule>` — apply rule top-down through the tree.
- `bottomup <rule>` — apply rule bottom-up through the tree.
- `oncetd <rule>` — apply rule to first match found top-down.
- `alltd <rule>` — apply rule to all matches, don't recurse into successes.
...
- `stringify` — convert a Selection back to an ABC string (header + body, via toAst then Formatter).
- `stringify-header` — stringify only the tune header (X: through K:, including V: declarations).
- `stringify-body` — stringify only the tune body (no header info lines). Used when assembling multi-voice output.
