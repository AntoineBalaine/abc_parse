...
Phase 2 is the critical validation point for context-sensitivity. Because ABC notation is context-sensitive (the same character means different things in different contexts), Phase 2 will reveal whether the TreeSitter grammar and scanner correctly handle context via the `valid_symbols` mechanism.
...
The TreeSitter scanner receives a `valid_symbols[]` array from the parser indicating which tokens are valid at the current parse position. This is how TreeSitter handles context-sensitivity:

- In lyric mode: `valid_symbols[TT_LY_HYPH] = true`, `valid_symbols[TT_TIE] = false`
- In music mode: `valid_symbols[TT_LY_HYPH] = false`, `valid_symbols[TT_TIE] = true`

The scanner checks `valid_symbols` before emitting context-sensitive tokens like `-` (tie vs lyric hyphen).
 ...
 * Only line_number is actively used; other fields exist for compatibility.
 */
...
All 77 scanner sub-functions that need tests:

| Category | Functions | Test File |
|----------|-----------|-----------|
| Whitespace/EOL | `scan_whitespace`, `scan_eol` | `test_whitespace.c` |
| Pitch/Notes | `scan_note_letter`, `scan_accidental`, `scan_octave`, `scan_rest`, `scan_tie`, `scan_decoration`, `scan_slur` | `test_pitch.c` |
...
| Info | `scan_info_header`, `scan_info_string`, `scan_info_continuation`, `scan_percent_token`, `scan_voice` | `test_info.c` |
| Symbols | `scan_slur`, `scan_annotation`, `scan_symbol`, `scan_chord_symbol`, `scan_escaped_char` | `test_symbol.c` |
| Whitespace | `scan_whitespace`, `scan_eol`, `scan_section_break` | `test_whitespace.c` |
| Special | `scan_ampersand`, `scan_system_break`, `scan_y_spacer`, `scan_backtick_spacer`, `scan_line_continuation` | `test_special.c` |
...
| Punctuation | `scan_equals`, `scan_slash`, `scan_minus`, `scan_plus`, `scan_lparen`, `scan_rparen`, `scan_lbrace`, `scan_rbrace`, `scan_lbracket`, `scan_rbracket`, `scan_pipe`, `scan_reserved_char`, `scan_invalid` | `test_punctuation.c` |

### Function Signatures

Most functions take only `TSLexer *lexer`, but some require additional parameters:

| Signature | Functions |
|-----------|-----------|
| `TSLexer *lexer, ScannerState *state` | `scan_eol`, `scan_section_break` |
| `TSLexer *lexer, const bool *valid_symbols` | `scan_percent_token`, `scan_info_header`, `scan_ampersand`, `scan_lyric_header` |

Example tests for functions with extra parameters:

```c
// For functions taking ScannerState
void test_scan_eol() {
  ...
  ASSERT_EQ(2, state.line_number);  // Line number should increment
}

// For functions taking valid_symbols
void test_scan_percent_token_comment() {
  TSLexer lexer = create_test_lexer("%comment\n");
  bool valid_symbols[TT_COUNT] = {0};
  valid_symbols[TT_COMMENT] = true;
...
```makefile
CC = cc
CFLAGS = -I ../src -I $(shell brew --prefix)/include -Wall -Wextra
...
This is the critical phase for validating context-sensitivity. Create JS tests in `tree-sitter-abc/tests/` that:
...
If token sequences differ for context-sensitive constructs (like `-` in lyrics vs music), it indicates either:
- The scanner is not checking `valid_symbols`
- The grammar is not constraining `valid_symbols` correctly
...
1. All 77 scanner sub-functions have unit tests
2. All tests pass
3. Any bugs found are fixed and committed
4. `npm run test:c` exits with code 0

### Phase 2 Complete When:
1. Token sequence tests pass for all TypeScript scanner test inputs
2. Context-sensitive constructs produce matching tokens
3. Mismatches are fixed in either scanner or grammar

### Phase 3 Complete When:
1. Full parse tree comparison passes
2. AST structure matches between TypeScript and TreeSitter parsers

## Phase Completion Workflow

After completing each phase, the implementer must follow this sequence before starting the next phase:

1. Run all tests (`npm run test:c` for Phase 1, or the appropriate test command for later phases)
2. Call the code review agent on the current phase's changes, asking whether the changes correctly implement the plan
3. Address any feedback from the code review agent
4. Run all tests again to ensure the feedback fixes do not introduce regressions
5. Commit the changes
6. Only then proceed to the next phase

Copy the plan file into the plans directory, and once done with the implementation of each of the phases of the plan, make sure to call the code review agent on the current phase and address any feedback that the code review agent might have. After done with the feedback and all the tests are passing, commit the changes and then move onto the next phase.

## To Do

- [ ] Create `tree-sitter-abc/tests/` directory
- [ ] Create `tree-sitter-abc/tests/test_harness.h`
- [ ] Create `tree-sitter-abc/tests/Makefile`
- [ ] Create test files for each category (15 files)
- [ ] Add `test:c` script to `tree-sitter-abc/package.json`
- [ ] Run Phase 1 tests and fix scanner bugs
- [ ] Code review Phase 1, address feedback, commit
- [ ] Implement Phase 2 token comparison tests
- [ ] Run Phase 2 and fix context-sensitivity issues
- [ ] Code review Phase 2, address feedback, commit
- [ ] Implement Phase 3 parse tree comparison
- [ ] Code review Phase 3, address feedback, commit
