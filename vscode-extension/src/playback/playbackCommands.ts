...
export function registerPlaybackCommands(context: vscode.ExtensionContext): void {
  ...
  const playDocumentCmd = vscode.commands.registerCommand("abc.playDocument", async () => {
    await manager.playCurrentDocument();
  });
  context.subscriptions.push(playDocumentCmd);

  // Play the selected text
  const playSelectionCmd = vscode.commands.registerCommand("abc.playSelection", async () => {
    await manager.playSelection();
  });
  context.subscriptions.push(playSelectionCmd);

  // Stop playback
  const stopPlaybackCmd = vscode.commands.registerCommand("abc.stopPlayback", async () => {
    await manager.stop();
  });
  context.subscriptions.push(stopPlaybackCmd);

  // Pause playback
  const pausePlaybackCmd = vscode.commands.registerCommand("abc.pausePlayback", async () => {
    await manager.pause();
  });
  context.subscriptions.push(pausePlaybackCmd);

  // Resume playback
  const resumePlaybackCmd = vscode.commands.registerCommand("abc.resumePlayback", async () => {
    await manager.resume();
  });
  context.subscriptions.push(resumePlaybackCmd);

  // Show available instruments
  const showInstrumentsCmd = vscode.commands.registerCommand("abc.showInstruments", async () => {
    const instruments = await manager.getInstruments();

    if (instruments.length === 0) {
      vscode.window.showInformationMessage("No instruments available. Install Muse Sounds via MuseHub.");
      return;
    }
    ...
    if (selected) {
      vscode.window.showInformationMessage(`Selected: ${selected.label} (ID: ${selected.id})`);
    }
  });
  context.subscriptions.push(showInstrumentsCmd);

  // Initialize playback on extension activation
  const initPlaybackCmd = vscode.commands.registerCommand("abc.initPlayback", async () => {
